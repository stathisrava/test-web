...
function clearPolylines() {
    // Clear individual marker polylines
    markers.forEach(marker => {
        if (marker.taskPolylinesGroup) {
            map.removeLayer(marker.taskPolylinesGroup);
            marker.taskPolylinesGroup.clearLayers();
        }
    }); 

    // Clear global polylines array
...
<br><br><br> <h2>Κατάσταση Αποθήκης</h2>
...
<!-- Add a filter for category names -->
<label for="categoryFilter">Filter by Category:</label>
<select id="categoryFilter" multiple>
  <!-- Populate this with your category names -->
...
$(document).ready(function() {
  ...
  $.ajax({
    ...
    success: function(categories) {
      ...
      var categoriesDropdown = $('#categoryFilter');
      categoriesDropdown.empty();
      $.each(categories, function(index, category) {
        categoriesDropdown.append('<option value="' + category.category_name + '">' + category.category_name + '</option>');
      });

      // Make an AJAX request for cargo and inventory data
      $.ajax({
        url: 'cargo_inventory.php',  // Change this URL to the correct path of your cargo_inventory.php file
        type: 'GET',
        ...
        success: function(data) {
          ...
          function filterItems(selectedCategories) {
            ...
            // Populate the table with received data based on selected categories
            $.each(data.data, function(index, item) {
              if ($.inArray(item.category, selectedCategories) !== -1) {
                $('#tasksTableBody1').append('<tr><td>' + item.source + '</td><td>' + item.itemId + '</td><td>' + item.itemName + '</td><td>' + item.quantity + '</td></tr>');
              }
            });
          }
          ...
          // Update the table when the category filter changes
      ...
      function fetchUserLocations() {
        $.ajax({
            ...
            data.forEach(function (user) {
                ...
                const marker = L.marker([user.latitude, user.longitude], {icon: greenMarkerIcon, tasks_id: user.tasks_id }).addTo(map);

                console.log("offer task id:",user.tasks_id);
                tasksIdsArray.push(user.tasks_id);
  ...
  function fetchUserreqLocations() {
        $.ajax({
            ...
            success: function (data) {
              ...
              // Add markers for each user location
            data.forEach(function (user) {
                const marker = L.marker([user.latitude, user.longitude],{ icon: redMarkerIcon, tasks_id: user.tasks_id }).addTo(map);

              redMarkersGroup.addLayer(marker);
              redMarkers.push(marker);

             console.log("request task id:",user.tasks_id);
             tasksIdsArray.push(user.tasks_id);
...
// Function to draw polylines for tasks
function drawPolylinesForTasks(marker, taskData) {
    // Clear existing polylines
    clearPolylines();

    console.log('got into function');
    console.log('taskdataarray:', taskData);
    console.log(tasksIdsArray);

    const tasksIdOfMarker = /*marker.options.tasks_id; */ /* user.tasks_id; */ //tasks_id;
    tasksIdsArray;

    console.log("tasksIdOfMarker:", tasksIdOfMarker); //marker is where??? check done it!

    if (!marker.taskPolylinesGroup) {
        marker.taskPolylinesGroup = L.layerGroup().addTo(map);
        markers.push(marker); // Add marker to the global markers array
    
    } else {
        marker.taskPolylinesGroup.clearLayers(); // Clear existing polylines if any
    }

    taskData.forEach(function (task) {
        if (tasksIdOfMarker.includes(task.tasks_id)) {
            ...
            }).addTo(marker.taskPolylinesGroup);
        }
    });

    // Add the polyline to a layer group for easier management
    if (marker.taskPolylinesGroup) {
        map.addLayer(marker.taskPolylinesGroup);
    }
}
...
function fetchRescuerLocations() {
    ...
    $.ajax({
        ...
        method: 'GET',
        success: function (rescuerData) {
            // Add markers for each rescuer location
            rescuerData.forEach(function (rescuer) {
                ...
                if (rescuer.active_tasks > 0) {
                    ...
                    marker.on('click', function () {
                        // Always remove the drawnPolylines when a marker is clicked
                        if (drawnPolylines) {
                            map.removeLayer(drawnPolylines);
                            drawnPolylines = null;
                        }

                        // Check if the clicked marker is the same as the selectedMarker
                        if (selectedMarker === marker) {
                            // If it is the same marker, toggle the removal or addition of drawnPolylines
                            if (!drawnPolylines) {
                                // Fetch and draw polylines for tasks
                                $.ajax({
                                    url: 'gettasksid.php',
                                    method: 'GET',
                                    data: { user_id: rescuer.users_id },
                                    success: function (taskData) {
                                        console.log(taskData);
                                        // Call the function to draw polylines for tasks
                                        drawPolylinesForTasks(marker, taskData);
                                    },
                                    error: function (error) {
                            ...
                            // If it's a different marker, update selectedMarker
